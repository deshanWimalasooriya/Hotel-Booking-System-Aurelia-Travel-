...
export default function Profile() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    email: user?.email || '',
    password: '',
    // If you need separate address fields:
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      // 1. Check which text fields changed
      ...
      // 4. Send PUT Request to your API
      ...
      if (response.data.success) {
        ...
        // Important: Update the global user context so headers/other components update immediately
        await refreshUser(); 
      }
  ...
  const handlePaymentInput = (e) => {
    const { name, value } = e.target;
    // Basic formatting logic (keep your existing logic if complex)
    if (name === 'card_number') {
    ...
    try {
        ...
        await axios.put(
            `http://localhost:5000/api/users/${user.id}`,
            {
                ...
                card_number: cleanNumber, // Ensure backend handles security/hashing if storing
                cvv: null, // Usually don't store CVV
